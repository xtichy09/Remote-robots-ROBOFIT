...
ROSLIB.Ros.prototype.connect = function(url) {
   ...
   * TODO - pri otvoreni registrace
   */
  function onOpen(event) {
    that.emit('connection', event);
    console.log("REGISTRATION");
    that.socket.send('REGISTRATION:WEBCLIENT|' + document.getElementById('login').innerHTML);

      var li = document.createElement("li");
      li.className = "left clearfix";
      var icon = document.createElement("span");
      icon.className = "chat-img pull-left";
      var icon2 = document.createElement("span");
      icon2.className = "glyphicon glyphicon-cloud-upload";
      icon.appendChild(icon2);
      li.appendChild(icon);
      chat.appendChild(li);

      var text = document.createElement("div");
      text.className = "chat-body clearfix";
      var head = document.createElement("div");
      head.className = "header";
      var t = document.createElement("strong");
      t.className = "primary-font";
      t.appendChild(document.createTextNode(document.getElementById('login').innerHTML));
      var p = document.createElement("p");
      p.appendChild(document.createTextNode('REGISTRATION:WEBCLIENT|' + document.getElementById('login').innerHTML));
  ...
  function onMessage(message) {
      ...
      var icon = document.createElement("span");
      icon.className = "chat-img pull-right";
      var icon2 = document.createElement("span");
      icon2.className = "glyphicon glyphicon-cloud-download";
      icon.appendChild(icon2);
      li.appendChild(icon);
      chat.appendChild(li);
      var text = document.createElement("div");
      ...
      head.className = "header";
      var t = document.createElement("strong");
      t.className = "pull-right primary-font";
      t.appendChild(document.createTextNode("xrobot00"));
      var p = document.createElement("p");
    ...
    if(parseData[0] == 'REGISTRATION:OK'){
      window.isRegistred = true;
      document.getElementById("rosbridgeinfo").innerHTML = "Online";
      console.log("OK JSEM REGISTROVANY");

    }else if(parseData[0] == 'STREAM:REGISTRATION'){
      var html = '';
      html += '<img src="http://37.205.11.196:'+parseData[1]+'/stream?topic=/stereo/left/image_rect_color" height="300" width="400"></img>';
      $("#video").html(html);   
    }else if(parseData[0] == 'STREAM:STOP'){
      var html = '';
      html += '<img src="no-video.jpg" height="300" width="400"></img>';
      $("#video").html(html);   
    }else if(parseData[0] == 'PONG:SERVER'){
      var currentdate = new Date();
      var time = parseData[1].split(':');
      var serverLatency = (currentdate.getSeconds() - parseInt(time[0])) * 1000 + (currentdate.getMilliseconds() - parseInt(time[1]));
      document.getElementById("serverPing").innerHTML = serverLatency;
      console.log(serverLatency);
      console.log(parseData);
    }else if(parseData[0] == 'PONG:ROBOT'){
      document.getElementById("robotPing").innerHTML = parseData[1];
      console.log(parseData);
    }else if(parseData[0] == 'PONG:WEBCLIENT'){
      var currentdate = new Date();
      var time = parseData[1].split(':');
      var robotLatency = (currentdate.getSeconds() - parseInt(time[0])) * 1000 + (currentdate.getMilliseconds() - parseInt(time[1]));
      document.getElementById("webclientPing").innerHTML = robotLatency;
      console.log(robotLatency);
      console.log(parseData);
    }else{
...
ROSLIB.Ros.prototype.callOnConnection = function(message) {
  ...
  if(typeof message != 'object'){
    ...
    if(message.split(':')[0] == 'PING')
    ...
    else if(message.split(':')[0] == 'START'){
      var messageJson = message;
    }else if(message.split(':')[0] == 'STOP'){
      var messageJson = message;
      ...
      var messageJson = 'MESSAGE:WEBCLIENT|' + JSON.stringify(message);
        ...
        t.appendChild(document.createTextNode(document.getElementById('login').innerHTML));
        var p = document.createElement("p");
        p.appendChild(document.createTextNode(messageJson));
...
ROSLIB.Ping = function(ros) {
  this.ros = ros;
}

ROSLIB.Ping.prototype.send = function(who, time) {
  this.ros.callOnConnection('PING:' + who + '|' + time.getSeconds().toString() + ':' + time.getMilliseconds().toString());
};
...
ROSLIB.TFClient = function(options) {
  ...
  this.transThres = options.transThres || 0.01;callOnConnection
